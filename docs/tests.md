...
|-------|----------|-------------|
| Tests automáticos | 325 funciones / 429 ejecuciones | pytest (sin Docker) |
...
Nota sobre ejecución: 15 de las 429 ejecuciones automáticas requieren Docker y Nginx levantados (`test_https_config.py` y `test_rate_limiting.py`). Sin Docker, pasan 414. Con Docker completo, pasan las 429.

---

## Sobre el conteo de tests

A partir del archivo `test_scheduler.py` (verificación del calendario del cron) el proyecto incluye tests parametrizados. Pytest cuenta cada caso parametrizado como una ejecución independiente, por lo que el número de **ejecuciones** (429) es mayor que el número de **funciones de test** escritas (325). Ejemplo:
...
pytest tests/ --collect-only -q | grep -c "::"   # total de ejecuciones (429)
```

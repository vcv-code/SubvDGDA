...
✔ El spinner deja de mover la página, y auditoría completa del README
  ...
  · **Tres decisiones de seguridad evaluadas y aplazadas a propósito**, documentadas para que no parezcan descuidos: los refresh tokens siguen en `localStorage` y no en cookies `httpOnly`, no hay cabecera `Content-Security-Policy`, y hay 44 usos de `innerHTML` con datos de la API sin función de escapado. **El razonamiento importa más que la conclusión**: migrar a cookies no es una mejora limpia sino un **intercambio** —elimina el robo de token por XSS pero introduce CSRF, que hoy no existe, y obliga a gestionar `SameSite`, revisar CORS y reescribir los tests de autenticación—, y una CSP estricta rompería lo que hay, porque en el frontend quedan 21 manejadores inline, dos `<script>` incrustados y 84 atributos `style=`. La exposición real es estrecha: no hay registro público, ni contenido escrito por usuarios, los scripts externos llevan SRI y el access token dura 15 minutos. Lo que se ha escrito, y es lo que de verdad sirve dentro de un año, son **los cuatro disparadores que invalidan el análisis** —reabrir el registro público, aceptar contenido de usuarios, añadir JavaScript de terceros sin SRI, o manejar datos más sensibles— y el **orden correcto** de acometerlo si llega el caso: escapado de `innerHTML` → CSP permisiva → CSP estricta → cookies, porque las dos primeras atacan la causa y sin ellas las cookies protegen menos de lo que parece

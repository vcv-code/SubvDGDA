...
| `metodo-cer.html` | **Qué es el método CER y por qué hay dinero público detrás**, en formato de preguntas y respuestas. Responde al *por qué* que la web no contaba: qué es un gato comunitario y sus grados de sociabilidad, por qué retirarlos o reubicarlos no funciona, por qué la marca de la oreja es el mal menor, por qué no basta con esterilizar —el programa sanitario completo es obligación legal desde 2023, no una mejora—, por qué no se testa de leucemia ni inmunodeficiencia de forma rutinaria, que el PIF ya tiene cura, quién sostiene el trabajo en la práctica y con cuánto dinero privado, qué obliga la Ley 7/2023 a cada administración, cómo se hace en otros sitios, y las dos objeciones que más se repiten —las ratas y el supuesto daño a la biodiversidad—. Cada afirmación con su fuente: BOE, DGDA, AVATMA, International Cat Care, AAFP, ABCD y los propios datos de esta web |
...
- `assets/` — recursos estáticos organizados en subcarpetas: `img/` (logo, error404), `img/home/` (imágenes de portada), `img/logos/` (logos de entidades), `docs/` (PDF que sirve la web en vez de enlazar fuera: la plantilla de propuesta al ayuntamiento, el proyecto de gestión de colonias, el tríptico de convivencia, el resumen del Plan de Acción 2026-2030 y el informe de FDCats —este último ajeno, alojado con su origen citado en la propia entrada, porque los enlaces a otras webs se mueren cuando las reorganizan), `geojson/` (fronteras de las CCAA para el mapa), `wireframes/` (capturas de diseño por pantalla), `guia-estilo/` (paleta, tipografía y PDF de wireframes)
...
| `make aviso-visitas` | Enseña el **correo mensual de visitas sin enviarlo**, para ver cómo queda. En el servidor lo lanza el `crontab` del sistema el día 1 de cada mes |
| `make evolucion` | **Evolución mes a mes**: gráfica de la media diaria por mes, comparación con el mes anterior y las páginas y procedencias de cada informe guardado. Sale del histórico, no de los registros. `ABRIR=no` para no abrir el navegador |
...
- **Histórico de visitas que solo crece** (`informes/historico.json`) — los registros de Nginx se borran a los 30 días, así que el resumen solo puede mirar ese último mes y lo anterior se pierde **para siempre**. El histórico guarda las visitas por día antes de que eso pase, y `make informes` lo alimenta con cada descarga: no hay que acordarse de nada, y si se pasan tres semanas el informe de 30 días rellena el hueco. Se sembró con los informes que ya había guardados, que llevan las cifras dentro — así se recuperaron los 41 días desde que la web salió a producción. **Es el único fichero de `informes/` que se versiona**, y a propósito: perderlo sería perder unos datos que no se pueden reconstruir. El servidor mantiene su propia copia por tarea programada semanal, y `make informes` se la trae y la fusiona con la local quedándose con el mayor valor de cada día — así, aunque se pasen dos meses sin mirar nada, no se pierde ninguno. Por eso se filtran al extraerlo las direcciones IP y las cadenas de ataque que llegan por la cabecera Referer, que es lo único personal que podría colarse
...
- **CSS consolidado** — una sola hoja de estilos con índice de las 35 secciones en la cabecera, **verificado por tests**: si se añade una sección al cuerpo y no se anota, la batería falla. Ninguna clase usada en el marcado se queda sin definir, también comprobado. Los números no son correlativos y cuatro están repetidos, y se han dejado así a propósito —la documentación cita las secciones por número en 47 sitios, uno de ellos un historial— con la razón escrita en el propio índice, para que no se «arregle» rompiendo las citas. De los `style=` inline que quedan, 68 de 82 son `display:none` para alternar visibilidad desde JavaScript; los 14 restantes (anchos de columna, márgenes puntuales y los colores de la leyenda del mapa) siguen siendo mejora pendiente. Las páginas nuevas ya no añaden ninguno: `metodo-cer.html` no tiene ni uno
